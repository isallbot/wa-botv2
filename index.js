/*
]=====> ISALL GANS <=====[ ]=====> YT ISALL GANS <=====[ ]=====> 089630267618 <=====[
*/
const {
   WAConnection,
   MessageType,
...
limitawal = 20
memberlimit = 2
cr = '*BOTZ ISALL VERIFIED*'

/*
]=====> SETTINGS <=====[
*/
const ownerNumber = ["6289630267618@s.whatsapp.net","6289630267618@s.whatsapp.net"]
const botName = 'BOTZ ISALL'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' MAKASIH UDAH ORDER BOT DI ISALL GANS'))
})
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply('Yang mau di tulis apaan? TOLOL?')
ramlan = body.slice(7)
reply('[❗] TUNGGU GOBLOK GW LAGI NULIS')
buff = await getBuffer(`https://api.vhtear.com/write?text=${ramlan}&apikey=${vhtear}`)
client.sendMessage(from, buff, image, {quoted: mek, caption: 'JANGAN LUPA BILANG MAKASIH:v'})
await limitAdd(sender)
break					
case 'quotes':
if (isBanned) return reply(ind.baned())
...
break
				...
				if (!isQuotedSticker) return reply('Reply atau Tag sticker yang mau dijadiin gambar kak >_<')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'JANGAN LUPA BILANG MAKASIH [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                ...
                var gh = body.slice(12)
					...
					client.sendMessage(from, buffer, image, {caption: 'JANGAN LUPA BILANG MAKASIH >_<', quoted: mek})
					await limitAdd(sender)
					break
                   case 'stalkig':
                   if (isBanned) return reply(ind.baned())
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner FAISAL WAPI`)
                      if (isOwner) {
                      const one = 999999999
